(() => {
  ...
  function render(pet) {
    ...
    detail.innerHTML = `<section class="detail-art"><p class="eyebrow">${pet.collectionNumber} / DROP 01</p><button class="detail-stage${pet.aura ? ' detail-stage--aura' : ''}" type="button" aria-label="Fazer carinho em ${pet.nickname}"><span class="detail-sprite" style="--sprite: url('${pet.asset}')" aria-hidden="true"></span><span class="detail-floor" aria-hidden="true"></span><span class="detail-pet-hint">CLICK TO PET ♥</span></button></section><section class="detail-copy"><p class="eyebrow">MEET YOUR CODEX PET</p><h1>${pet.nickname}</h1><p class="detail-description">${pet.description}</p><ul class="trait-list">${traits}</ul><article class="install-card"><div class="install-card__header"><div><span class="install-card__step">READY TO MOVE IN</span><h2>Install ${pet.nickname}</h2></div><span class="format-badge">CODEX PET V2</span></div><a class="install-primary" href="${codexUrl(pet)}">INSTALL IN CODEX <span>↗</span></a><div class="command-tabs" role="tablist" aria-label="Sistema operacional"><button class="is-active" type="button" data-os="windows" role="tab" aria-selected="true">WINDOWS</button><button type="button" data-os="unix" role="tab" aria-selected="false">MAC / LINUX</button></div><div class="command-box"><pre><code id="install-command"></code></pre><button id="copy-command" type="button">COPY</button></div><a class="download-link" href="${releaseUrl(pet)}" download>DOWNLOAD .CODEX-PET.ZIP <span>↓</span></a><ol class="install-steps"><li>Execute o comando acima.</li><li>Abra <strong>Settings → Pets</strong> no app.</li><li>Selecione <strong>Refresh</strong> e escolha ${pet.nickname}.</li></ol></article></section>`;
